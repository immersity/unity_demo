...
public class MagneticClick {
	...
	public float pressedThreshold = 0.10f;
	public float releasedThreshold = 0.03f;
	...
	public void magUpdate(Vector3 acc,  Vector3 compass) {
		...
		if (tiltedMagn < 0.2 && (magnetMagn / magnetBaseLine) < (1.0f - pressedThreshold)) {
			pressedDown = true;
		}

		
		if (tiltedMagn < 0.2 && (magnetMagn / magnetBaseLine) > (1.0f + releasedThreshold)) {
			pressedDown = false;
		}
